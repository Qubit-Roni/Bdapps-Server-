...
export default function OtpLogs() {
  ...
  const fetchLogs = async () => {
    setIsLoading(true);
    try {
      let url = `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/v1/otp/logs`;
  ...
  const verifiedCount = filteredLogs.filter(l => l.status === 'verified').length;

  const downloadPDF = () => {
    try {
      const doc = new jsPDF();
      doc.text("OTP Logs Report", 14, 15);
      
      const tableColumn = ["Sl", "Phone", "App ID", "OTP Code", "Status", "Retry", "Date"];
      const tableRows: any[] = [];
      ...
      doc.save(`OTP_Logs_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (error) {
    ...
    <PortalLayout>
      <div className="max-w-full mx-auto space-y-6">
        ...
        <div className="bg-gradient-to-r from-[#337ab7] to-[#286090] rounded-xl shadow-md p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <h2 className="text-white text-lg sm:text-xl font-bold tracking-wide">OTP Logs</h2>
            <p className="text-white/80 text-sm mt-1">
              Total Verified: <span className="font-bold text-white bg-white/20 px-2 py-0.5 rounded ml-1">{verifiedCount}</span>
            </p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white border-b border-gray-200">
                  ...
                  <th className="py-3 px-4 md:py-4 md:px-6 text-xs md:text-[15px] font-bold text-gray-800">OTP Code</th>
                  <th className="py-3 px-4 md:py-4 md:px-6 text-xs md:text-[15px] font-bold text-gray-800">Status</th>
                  <th className="py-3 px-4 md:py-4 md:px-6 text-xs md:text-[15px] font-bold text-gray-800">Retry</th>
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {isLoading ? (
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-gray-500">Loading logs...</td>
                  </tr>
                ) : filteredLogs.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-gray-500">No OTP logs found.</td>
                  </tr>
                ) : (
                  filteredLogs.map((log, idx) => (
                    <tr key={log._id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4 md:py-5 md:px-6 text-sm font-mono text-gray-700">
                        {log.otpCode}
                      </td>
                      <td className="py-3 px-4 md:py-5 md:px-6 text-sm">
                        {log.status === 'verified' ? (
                          <span className="text-green-600 bg-green-50 px-2 py-1 rounded font-semibold">Verified</span>
                        ) : log.status === 'failed' ? (
                          <span className="text-red-600 bg-red-50 px-2 py-1 rounded font-semibold">Failed</span>
                        ) : log.status === 'expired' ? (
                          <span className="text-gray-600 bg-gray-100 px-2 py-1 rounded font-semibold">Expired</span>
                        ) : (
                          <span className="text-orange-600 bg-orange-50 px-2 py-1 rounded font-semibold">Pending</span>
                        )}
                      </td>
                      <td className="py-3 px-4 md:py-5 md:px-6 text-sm text-gray-600">
                        {log.retryCount}
