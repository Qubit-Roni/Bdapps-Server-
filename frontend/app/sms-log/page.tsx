...
export default function SmsLog() {
  ...
  const fetchLogs = async () => {
    setIsLoading(true);
    try {
      // Assuming api-gateway proxies /api/v1/sms to sms-service
      let url = `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/v1/sms/all`;
      ...
      if (res.ok && data.success) {
        ...
        toast.error(data.message || "Failed to fetch SMS logs");
      }
    } catch (err: any) {
  ...
  const downloadPDF = () => {
    try {
      ...
      const tableColumn = ["Sl", "Phone", "App ID", "Message", "Status", "Date"];
      const tableRows: any[] = [];
      ...
      doc.save(`SMS_Logs_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (error) {
    ...
    <PortalLayout>
      <div className="max-w-full mx-auto space-y-6">
        ...
        <div className="bg-gradient-to-r from-[#337ab7] to-[#286090] rounded-xl shadow-md p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <h2 className="text-white text-lg sm:text-xl font-bold tracking-wide">SMS Logs</h2>
            <p className="text-white/80 text-sm mt-1">
              Total Sent: <span className="font-bold text-white bg-white/20 px-2 py-0.5 rounded ml-1">{sentCount}</span>
            </p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white border-b border-gray-200">
                  ...
                  <th className="py-3 px-4 md:py-4 md:px-6 text-xs md:text-[15px] font-bold text-gray-800">Message Content</th>
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="py-8 text-center text-gray-500">No SMS logs found.</td>
                  </tr>
                ) : (
                  filteredLogs.map((log, idx) => (
                    <tr key={log._id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4 md:py-5 md:px-6 text-sm text-gray-700 max-w-sm">
                        <div className="max-h-24 overflow-y-auto pr-2 scrollbar-thin whitespace-pre-wrap">
                          {log.message}
                        </div>
                      </td>
                      <td className="py-3 px-4 md:py-5 md:px-6 text-sm">
                        {log.status === 'sent' ? (
                          <span className="text-blue-600 bg-blue-50 px-2 py-1 rounded font-semibold">Sent</span>
                        ) : log.status === 'failed' ? (
                          <span className="text-red-600 bg-red-50 px-2 py-1 rounded font-semibold">Failed</span>
                        ) : (
                          <span className="text-orange-600 bg-orange-50 px-2 py-1 rounded font-semibold">{log.status}</span>
                        )}
                      </td>
